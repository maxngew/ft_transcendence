import { beforeEach, describe, expect, mock, test } from "bun:test";

import { internalRealtimeSecretHeader } from "../../shared/realtime-internal";
import { handleInternalChallengeReceived } from "./internal-challenge-received";

const emit = mock((_event: string, _payload: unknown) => {});
const to = mock((_room: string) => ({ emit }));
const log = mock((_message: string) => {});

const payload = {
  declineToken: "decline-token",
  matchId: "match-1",
  password: "room-password",
  senderUsername: "black",
  username: "white",
};

function jsonRequest(body: unknown, secret = "shared-secret") {
  return new Request("http://realtime/internal/challenge-received", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      [internalRealtimeSecretHeader]: secret,
    },
    body: JSON.stringify(body),
  });
}

beforeEach(() => {
  emit.mockReset();
  to.mockReset();
  log.mockReset();
  to.mockImplementation(() => ({ emit }));
});

describe("handleInternalChallengeReceived", () => {
  test("rejects requests without the shared internal secret", async () => {
    const response = await handleInternalChallengeReceived(jsonRequest(payload), { to }, null);
    const responsePayload = await response.json();

    expect(response.status).toBe(503);
    expect(responsePayload).toEqual({ error: "internal_secret_unconfigured" });
    expect(to).not.toHaveBeenCalled();
  });

  test("rejects malformed payloads without broadcasting", async () => {
    const response = await handleInternalChallengeReceived(
      jsonRequest({ ...payload, declineToken: "" }),
      { to },
      "shared-secret",
      { log },
    );

    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "invalid_payload" });
    expect(to).not.toHaveBeenCalled();
  });

  test("broadcasts valid challenge receive events to the invitee", async () => {
    const response = await handleInternalChallengeReceived(
      jsonRequest(payload),
      { to },
      "shared-secret",
      { log },
    );
    const responsePayload = await response.json();

    expect(response.status).toBe(200);
    expect(responsePayload).toEqual({ ok: true, room: "user:white" });
    expect(to).toHaveBeenCalledWith("user:white");
    expect(emit).toHaveBeenCalledWith("challenge:receive", {
      declineToken: "decline-token",
      matchId: "match-1",
      password: "room-password",
      senderUsername: "black",
    });
    expect(log).toHaveBeenCalledTimes(1);
  });
});
